...
- `-o/--output <character|split|unified|json>` — default `character` (style50-parity presentation: banner, per-file headers, char-level colors — see `u50_style/AGENTS.md`, 'Character mode'); `json` also exists in the original (its README documents it; readthedocs was stale). Exit codes: 0 clean, 1 violations, 3 any per-file error (unreadable file, unsupported type, formatter failure) — other files are still checked and their output streamed.
