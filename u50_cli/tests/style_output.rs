...
fn run_mode_dirty_exits_1_and_prints_diff() {
    ...
    // Character mode (the default): banner, then the char-diff block, in
    // which the deleted and inserted texts are inlined at the diff
    // positions (`x` deleted, `X` inserted, the rest equal) — no +/- rows.
    assert!(
        stdout.contains("Results generated by u50 v"),
        "banner must print: {stdout}"
    );
    assert!(
        stdout.contains("xX = 1"),
        "char diff must inline both texts: {stdout}"
    );
    assert!(
        !stdout
            .lines()
            .any(|l| l.starts_with('+') || l.starts_with('-')),
        "character mode renders no +/- rows: {stdout}"
    );
...
fn run_mode_clean_exits_0_and_reports_looks_good() {
    ...
    assert!(
        ...
        "clean file must be reported: {stdout}"
    );
    assert!(
        !stdout
            .lines()
            .any(|l| l.starts_with('+') || l.starts_with('-')),
        "no diff rows for a clean file: {stdout}"
    );
    assert!(stderr.is_empty(), "no errors expected: {stderr}");
    cleanup(&path);
}
...
fn missing_file_errors_to_stderr_while_valid_file_diff_still_prints() {
    ...
    // Character mode: the valid file's diff (inlined del/ins text for the
    // single-char change, followed by the equal ` = 1`) must survive.
    assert!(
        stdout.contains("X = 1"),
...
fn check_mode_walk_warns_unknown_file_type_to_stderr_and_exits_1() {
    ...
    // Character mode (the default): banner plus the char-diff block — the
    // original and styled texts are inlined at the diff positions, so
    // neither appears as a verbatim +/- row. Equal runs (the braces and
    // `0;`, unchanged by the stub) render verbatim.
    assert!(
        stdout.contains("Results generated by u50 v"),
        "the banner must print on stdout: {stdout}"
    );
    assert!(
        !stdout
            .lines()
    ...
    assert!(
        ...
        "the C diff block must print on stdout: {stdout}"
    );
    assert!(
