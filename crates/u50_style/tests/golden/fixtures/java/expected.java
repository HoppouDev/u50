...
public class Mandelbrot extends JFrame {
    ...
    public static void main(String[] args)
    {
        ...
        // new Mandelbrot(800, 600, 4.5876514379235943e-09, -0.6092161175392330,
        // -0.4525577210859453); new Mandelbrot(800, 600, 5.9512354925205320e-10,
        // -0.6092146769531246, -0.4525564820098262); new Mandelbrot(800,
        // 600, 6.7178527589983420e-08, -0.7819036465400592, -0.1298363433443265); new
        // Mandelbrot(800, 600, 4.9716091454775210e-09, -0.7818800036717134, -0.1298044093748981);
        // new Mandelbrot(800, 600, 7.9333341281639390e-06, -0.7238770725243187,
        // -0.2321214232559487);
        /*
    ...
    public Mandelbrot(int width, int height, double[]... points)
    {
        ...
        setBounds(rint((screen.getWidth() - width) / 2), rint((screen.getHeight() - height) / 2),
                  width, height);
    ...
    private MouseAdapter mouseAdapter = new MouseAdapter() {
        ...
        private Consumer<Integer> setMdXY =
            v -> mdY = mpY + rint(abs((mdX = v) - mpX) / widthHeightRatio) * signum(mdY - mpY);
        private Consumer<Integer> setMdYX =
            v -> mdX = mpX + rint(abs((mdY = v) - mpY) * widthHeightRatio) * signum(mdX - mpX);
        public void mouseReleased(MouseEvent e)
        {
            if (e.getX() == mpX && e.getY() == mpY)
                return;
            stack.push(ctrl);
            if (!ctrl)
            {
    ...
    @Override public void paint(Graphics g)
    {
        ...
        // g.drawLine(insets.left+width/2, insets.top+0,        insets.left+width/2,
        // insets.top+height); g.drawLine(insets.left+0,       insets.top+height/2,
        // insets.left+width,   insets.top+height/2);
        if (!ctrl)
            return;
    ...
    private int color(double r0, double i0, int max, boolean straight, float shift)
    {
        ...
        while (++n < max && r2 + i2 < 4);
        return n == max ? black.getRGB()
                        : HSBtoRGB(shift + (float) (straight ? n : max - n) / max * 11 / 12f +
                                       (straight ? 0f : 1 / 12f),
                                   1, 1);
    }
}
