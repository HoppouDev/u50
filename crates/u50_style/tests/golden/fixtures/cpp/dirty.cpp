...
int main() {
  ...
  for (int a = 0; a < 255; ++a) {
    pal[a][0] =
        (uint8_t)round(127.0 + 127.0 * cos(2.0 * PI * a / 255.0)); // Blue
    pal[a][1] =
        (uint8_t)round(127.0 + 127.0 * sin(2.0 * PI * a / 255.0)); // Green
    pal[a][2] =
        (uint8_t)round(127.0 + 127.0 * sin(2.0 * PI * a / 255.0)); // Red
  }
  pal[255][0] = 255;
  pal[255][1] = 255;
  pal[255][2] = 255;
  cout << "Step 1: Rendering Mandelbrot using 80-bit long double (" << targetW
       << "x" << targetH << ")..." << endl;
  ...
  for (int y = 0; y < targetH; ++y) {
    ...
    for (int x = 0; x < targetW; ++x) {
      ...
      for (int j = 0; j < scale; ++j) {
        ...
        long double c_im =
            ry + (long double)((long long)b - (rawH / 2)) * step_d;
        for (int i = 0; i < scale; ++i) {
          size_t a = (size_t)x * scale + i;
          long double c_re =
              rx + (long double)((long long)a - (rawW / 2)) * step_d;
