...
{
    document_.assign(std::istreambuf_iterator<char>(is), std::istreambuf_iterator<char>());
...
bool OurReader::readValue()
{
    ...
    if (nodes_.size() > features_.stackLimit_)
    {
        ...
        token.end_ = current_;
        token.type_ = tokenError;
        return addError("Exceeded stackLimit for nested object and/or array values.", token);
#endif
    }
    readTokenSkippingComments(token);
    bool successful = true;
...
bool OurReader::readObject(Token &token)
{
    ...
    while (readTokenSkippingComments(tokenName))
    {
        if (tokenName.type_ == tokenObjectEnd && (name.empty() || features_.allowTrailingCommas_))
            return true;
        name.clear();
        if (tokenName.type_ == tokenString)
        {
...
bool OurReader::readArray(Token &token)
{
    ...
    for (;;)
    {
        ...
        if (current_ != end_ && *current_ == ']' &&
            (index == 0 ||
             (features_.allowTrailingCommas_ && !features_.allowDroppedNullPlaceholders_)))
        {
            Token endArray;
            readToken(endArray);
            return true;
