...
- Printing policy (in `fix`, engine-side like `run()`): plain fix prints per file `fixed: <path>` or `already clean: <path>` to stdout (diff rendering ignored); dry run in text modes prints the same per-file status lines (`would fix: <path>` / `already clean: <path>`) followed by the rendered diff for each file that would change, respecting `req.output`; dry run in JSON mode prints ONLY the JSON document of would-fix results (no status lines — the output is machine-readable). Errors always go to stderr as `error: <path>: <message>`.
