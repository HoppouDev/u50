...
API: `Language` — a cheap handle to a registered `LanguagePlugin`; languages are **plugins** registered in `crate::registry::languages()` (one module per language owning its metadata, comment counter, and formatter — adding a language is one module + one registration line), `detect_language` resolves by extension through the registry, and the handle exposes `required_tool() -> &'static str` / `pip_package() -> Option<&'static str>` (`None` for rustfmt), `Formatter` trait, `Cs50Formatter` (`Default` = built-in tools resolved cache-only), `Request { files, output, color }`, `FileResult { path, clean, rendered, formatted }`, `Report { results, errors }` with `clean()` and `has_errors()`. Pure renderers (`render_character`/`render_split`/`render_unified`/`json_document`) take `(source, formatted, ...)` and are unit-testable without the tools.
...
- A **directory** argument is walked recursively; only **regular files** whose `detect_language` is `Some` are collected (the extension filtering style50 applies while walking; FIFOs/devices and symlinked entries inside the tree are skipped — no blocking opens). Every **unsupported regular file** encountered during the walk emits the style50-parity warning `unknown file type "<path>", skipping...` — on stderr in text/fix modes, on stdout in score mode (before the score, matching the original's score-mode output); JSON stays silent, and the warning never affects exit codes. **Hidden directories are included** — in the original, exclusion is `--ignore`'s job (e.g. skipping `node_modules`), which u50 has not implemented yet.
- **Symlink parity** (verified against `os.walk` and style50 3.0.0, see STYLE50_V3_CROSSCHECK.md register #5): a symlinked **directory operand** is followed and walked (os.walk resolves the top-level operand); symlinked **subdirectories inside the walk** are never descended into (`followlinks=false`); symlinked **regular files** are collected and their targets read through the link, because style50 filters by name.
...
- The final file list is **deduplicated by canonical path** (a directory and a file inside it may both be named, and equivalent spellings like `dir` and `./dir/a.c` resolve to the same file; unresolvable paths fall back to their raw spelling) and **sorted** for deterministic output, applied identically to check and `--fix` (walk warnings are deduplicated the same way).
- **Parallelism:** the check pass (`check_files`, shared by `run_with`/`run_with_renderer`) processes files in parallel with **rayon**; rayon preserves input order, so results, errors, and rendering stay byte-identical to a sequential run. A **cold-cache provisioning pre-pass** (`provision_backends`) runs _after_ the walk and _before_ any file is processed (the walk itself only classifies files — it never fetches): it collects the distinct missing `(pip package, tool)` pairs of the discovered files and fetches them all **in parallel** through the same uv pipeline `--setup` uses (one fetch task per package, one serialized venv install — so parallel first-use cannot race concurrent uv installs into the shared cache; a cross-process advisory lock on the cache serializes whole provisioning runs, and lazy-provisioning progress goes to **stderr** so stdout stays pure diff/JSON). `fix_with` gets the same pre-pass and stays sequential for the in-place writes. `run()` prints rendered output for every processed file to stdout, then writes each error to stderr as `error: <path>: <message>`. Formatter-level failures (e.g. missing clang-format) are therefore per-file skips, not whole-run bails.
...
- Write policy: a file is written only when it is dirty, `dry_run` is false, and no error occurred. Clean files and dry runs never touch the filesystem. Writes are atomic (sibling temp file + rename): a failed or interrupted write leaves the original byte-for-byte intact, and a read-only target fails before anything is written (matching the EACCES the old in-place write produced); write failures are recorded in `Report.errors` as `could not write \`<path>\`: ...` (no bail).
...
`ScoreRenderer` (`Output::Score`) reproduces the original style50's score mode: per successfully processed file it accumulates half the inserted/deleted line count of the source-vs-styled line diff (the same `line_diff` machinery the display modes use) into `diffs`, and the styled text's non-blank line count into `lines`; `finish` prints one error line per errored file in order (bare message — without the `error: <path>: <message>` prefix — crossterm `DarkYellow` foreground when color is on, mirroring the original's unconditional termcolor yellow; the palette itself is emitted through **crossterm** — see 'Terminal rendering'), then the uncolored score `max(1 - diffs/lines, 0)` (`0.0` when nothing was checked successfully, i.e. only successful files contribute). A styled text with no non-blank lines contributes a `file is empty` error line instead of touching the sums (the original raises a per-file `Error` there). `lines` counts ALL lines of the styled text for Python (the original's
...
### Character mode (style50 parity)

The default `-o character` reproduces style50 3.0.0's `to_ansi` presentation (not a `+/-` diff): a bold bright-white banner `Results generated by u50 v<version>` emitted once, followed directly by the next element (u50 branding — a documented divergence from style50's `style50 v` banner); the blank line before a dirty file's diff body comes from the diff block itself; when the run covers **more than one file** (results and errors alike), a cyan `:::::::::::::: name ::::::::::::::` header per file; dirty files re-render the **original text** with character-level `on_red`/`on_green` background highlighting (inserted newlines/tabs shown as literal `\n`/`\t` markers, deleted newlines merging lines), followed by legend lines `<marker> means that you should insert|delete a newline|tab.` (deduplicated, first-seen order — per-file scope; the original keeps a run-wide, sorted set, so multi-file runs print fewer legend lines than style50 — documented divergence); clean files print a green `Looks good!`. Blank-line scaffolding is identical to the original. `--color never` and piped stdout emit **zero** ANSI escapes (u50 convention; style50 leaks bare resets when piped). The comments hint (style50 emits "And consider adding more comments!" for files with comments) is still not implemented — roadmap. Character alignment may differ from Python's `difflib` in ambiguous cases; structure and colors match (verified live, see [STYLE50_V3_CROSSCHECK.md](STYLE50_V3_CROSSCHECK.md), register #6).
...
4. **Parallel downloads**: one task and one spinner per package (indicatif
   ...
   dependencies (`TRANSITIVE_DEPS`) are fetched alongside it, pinned too
   (a dep-chain change would otherwise fail only as a per-file
   `ImportError` while provisioning reports success). Previously fetched
   wheels are reused across runs (re-verified against the sha256 sidecar).
   Formatter subprocesses run with `PYTHONHOME`/`PYTHONPATH`/`PYTHONSTARTUP`/
   `PYTHONPYCACHEPREFIX` stripped (the venv is self-contained; a user's
   conda/pyenv env must not shadow the pinned deps), each with a
   wall-clock timeout (`U50_STYLE_TOOL_TIMEOUT_SECS`, default 60s —
   documented divergence: style50 waits forever).
...
An empty or whitespace-only file carries no content after normalization (`""` for a truly empty file; a single `\n` when whitespace spans several lines) and is reported as a **per-file error** `file is empty` (style50 3.0.0 raises the same error when `count_lines` is 0; its default `count_lines` ignores blank lines). This replaces the earlier behavior of reporting such files clean.
...
- **Presentation divergences (cosmetic)**: style50 prints a "Results generated by style50 vX" banner, "Looks good!", comment-count hints ("But consider adding more comments!" — now implemented, see 'Character mode (style50 parity)' above), and a "\n means insert a newline" legend in character mode; u50's character mode now reproduces that presentation too (with a u50-branded banner — see 'Character mode (style50 parity)' above); style50's unified mode is not a patchable git diff (no `@@`/`---`/`+++`), u50's is.
