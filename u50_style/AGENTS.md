...
- comment-count hints (style50's "But consider adding more comments!" suggestion). Character-mode presentation now matches the original (banner, per-file headers, char-level colors — see 'Character mode (style50 parity)' above); only the hint remains.
