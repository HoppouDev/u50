...
- **u50_check/, u50_style/, u50_submit/**: library crates, one per original cs50 tool. Engines are stubs: `run()` currently bails with a "not implemented yet" error; request types (Request/enums) are defined and dispatched from the CLI. Each has its own `AGENTS.md` with program-specific details.
