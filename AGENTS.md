...
- **u50_check/, u50_style/, u50_submit/**: library crates, one per original cs50 tool. `u50_style`'s engine is implemented (the style50 3.0.0 language set — C/C++/Java via clang-format, Python via autopep8, JavaScript via js-beautify, HTML via djhtml, CSS via css-beautify, SQL via sqlformat, plus Rust via rustfmt (a u50 addition — rustfmt is not pip-installable and resolves from the Rust toolchain, never PATH); unified/character/split/json/score/html output; in-place fix (`--fix`) with dry-run; directory arguments; root-level `--status` table and `--setup`/lazy auto-provisioning of backends into a uv-managed venv in µ50's cache (`~/.cache/u50/style50` on Unix, `%LOCALAPPDATA%\u50\style50` on Windows; in-process via uv library crates — no system python3/pip/uv binary needed; cache-only tool resolution (plus the Rust toolchain for rustfmt), `PATH` never consulted). `u50_check`'s engine is implemented (a plugin-registry port of check50: local/offline/dev modes over a `.cs50.yaml` check directory, native "simple" YAML check pipelines plus a Rust check-authoring API for native check-set plugins under `checks/` registered in `registry.rs`, dependency-graph scheduling with per-check process-thread isolation, filesystem inheritance, timeouts, and the skip cascade, and ansi/json renderers byte-shaped to check50's documented output — remote/online mode and html output remain documented divergences). `u50_submit` remains a stub: `run()` bails with a "not implemented yet" error; request types (Request/enums) are defined and dispatched from the CLI. Each has its own `AGENTS.md` with program-specific details.
