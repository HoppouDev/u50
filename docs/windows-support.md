...
4. Wheel ranking rejects Windows wheels: setup.rs:72-77 ranks only any/manylinux/linux\_\* (wheel_rank at setup.rs:144-161); clang-format ships only platform wheels, so provisioning installs nothing on Windows. Unit test setup.rs:940-953 asserts win_amd64 is rejected; must flip.
...
Status: implemented-pending-CI-verification (.github/workflows/rust.yml). Added strategy.matrix.os [ubuntu-latest, windows-latest] with runs-on: ${{ matrix.os }}; moved U50_STYLE_GOLDEN=1 into the golden step's env: block (an inline `VAR=1 cmd` prefix is bash-only and breaks under pwsh). Golden policy: goldens gated to ubuntu (`if: runner.os == 'Linux'`) after observed Windows byte-drift in the djhtml (html) and SQL backends (html_golden, sql_golden, expected-fixture-clean failures in CI while c/cpp/java/py/js pass); CRLF is normalized in-engine, engine.rs:44. The Windows leg still enforces build/test/fmt/clippy; tool-versions.txt and fixtures are unchanged. The harness gate is now a dedicated ubuntu-only `harness` job using `paladini/harness-score@v1` with `min-level: '4'` and a self-updating badge published to the `badges` branch (replacing the `npx` step that ran in the matrix).
